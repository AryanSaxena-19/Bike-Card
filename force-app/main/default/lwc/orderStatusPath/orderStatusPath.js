...
export default class OrderStatusPath extends LightningElement {
    ...
    getPicklistValuesWired({ error, data }) {
        if (data) {
    ...
    getRecordWired({ error, data }) {
        if (data) {
            ...
            if (data.recordTypeInfo) {
                this.recordTypeI = data.recordTypeInfo.recordTypeId;
            }else {
    ...
    refreshPathItems() {
        ...
        let isCompleted = 
            this.picklistValue !== undefiend && this.picklistValue !== null;
