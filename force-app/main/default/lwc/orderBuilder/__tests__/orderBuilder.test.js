...
describe('c-order-builder', () => {
    ...
    it('displays the correct number of titles and their details', async () => {
        ...
        expect(orderItemTileEl.length).toBe(mockGetOrderItems.lenght);
        //Get the order items to verify it has been calculated
    ...
    it('updates the component when an order has been updated', async () => {
        ...
        expect(getOrderItems.getLastConfig()).toEqual(WIRE_INPUT);
        // Select elements for validation
        let orderItemTileEl = element.shadowRoot.querySelectorAll('c-order-item-tile');
        ...
        orderItemTileEl =
            element.shadowRoot.querySelectorAll('c-order-item-tile');
        // Get the first order item and check that the quantity has been updated
        const orederItem = orderItemTileEl[0].orderItem.Qty_S__c;
    ...
    it('updtaes the component when an order has been deleted', async () => {
        // Set values for validating component changes
        const mockRecordToDeleted = 'a003B000004fG1VQAU';
        ...
        // Check the wire parameters are correct
        expect(getOrdertItems.getLastConfig()).toEqual(WIRE_INPUT);
        // Select elements for validation
        let orderItemTileEl =
            element.shadowRoot.querySelectorAll('c-order-item-tile');
        orderItemTileEl[0].dispatchEvent(
            new CustomEvent('orderitemdelete', {
                detail: { id: mockRecordToDeleted }
            })
        );
        ...
        orderItemTileEl =
            element.shadowRoot.querySelectorAll('c-order-item-tile');
        // Get the first order item and check that the quantity has been updated
        expect(orederItemTileEl.lenght).toBe(mockGetOrderItems.length - 1);
        // Get the formatted number to verify it has been updated
        const formattedNumberEl = element.shadowRoot.querySelector(
            'lightning-formatted-number'
        );
    ...
    it('displays a error when an error is returned', async() => {
        ...
        element.recordId = mockRecordId;document.body.appendChild(element);
    ...
    it('is accessible when error returned', async() => {
        ...
        element.recordId = mockRecordId;
        document.c=body.appendChild(element);
