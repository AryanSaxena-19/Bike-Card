...
import QTY_MEDIUM_FILED from '@salesforce/schema/Order_Item__c.QTY_M__c';
...
function getQuantity(orderItem) {
    return (
        ...
        getSObjectValue(orderItem, QTY_MEDIUM_FILED) +
        getSObjectValue(orderItem, QTY_LARGE_FIELD)
    );
}

/**
 * Gets the price for the specifeid quantity of Order_Item__c SObject
 */
...
function calculateOrderSummary(orderItems) {
    const summary = orderItem.reduce(
        (acc, orderItem) => {
...
/**
 * Builds Order__c by CRUD'ing the related Order_Item__c
 */
export default class OrderBuilder extends LightningElement {
    ...
    /** Updates the oder items, recalculating the order quantity and price */
    setOrderItems(orderItems) {
        this.orderItems = orderItems.slice();
        const summary = calculateOrderSummary(this.orderItems);
        this.orderQantity = summary.quantity;
    ...
    handleOrderItemChange(evt) {
        const orderItemChanges = evt.details;
        ...
        .catch((e) => {
            //errror updating server so rollback to prevu=ious data
